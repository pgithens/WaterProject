...
window.onload = function() {
  ...
  document.getElementById("jello").onchange = function(event) {
      var jelloButton = event.target.value;
      if (jelloButton ) {
        liquidSpeed = 1.0;
        liquidWaves = 0.7;
        //gravity -=2;
  ...
  function update(seconds) {
    ...
    if (mode == MODE_MOVE_SPHERE) {
      ...
      switch (shapeType){
        ...
        case "honey":
          var percentUnderWater = Math.max(0, Math.min(1, (radius - center.y) / (2 * radius)));
          velocity = velocity.add(gravity.multiply(seconds - 1.1 * seconds * (percentUnderWater*2)));
          velocity = velocity.subtract(velocity.unit().multiply(percentUnderWater * seconds * velocity.dot(velocity)));
          center = center.add(velocity.multiply(seconds));
          if (velocity.y > 0 ){
        ...
        case "jello":
        if (center.y < 0.0){
          velocity= velocity.multiply(0.0);
        }
        else{
          ...
          if (center.y > 0.7 && velocity.y > 0.0){
