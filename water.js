...
function Water() {
  ...
  this.updateShader = new GL.Shader(vertexShader, '\
    ...
    uniform vec2 delta;\
    varying vec2 coord;\
    /* VARIABLES TO CONTROL THE LIQUID VISCOSITY*/\
    uniform float u_liquidSpeed;\
    uniform float u_liquidWaves;\
    void main() {\
      /* get vertex info */\
      vec4 info = texture2D(texture, coord);\
      \
      ...
      /* CHANGING THE MAP POINT - EFFECTS SPEED OF LIQUID- DONT MAKE ABOVE 5 change the velocity to move toward the average */\
      info.g += (average - info.r) * u_liquidSpeed;\
      \
      /* CHANGE IMPACT THE OBJECT HAS WHEN CONTACT MADE DONT MAKE ABOVE 1 attenuate the velocity a little so waves do not last forever */\
      info.g *= u_liquidWaves;\
      \
...
Water.prototype.stepSimulation = function(liquidSpeed, liquidWaves) {
  var this_ = this;
  this.textureB.drawTo(function() {
    this_.textureA.bind();
    this_.updateShader.uniforms({
      delta: [1 / this_.textureA.width, 1 / this_.textureA.height],
      u_liquidSpeed: liquidSpeed,
      u_liquidWaves: liquidWaves
    }).draw(this_.plane);
  });
  this.textureB.swapWith(this.textureA);
};
